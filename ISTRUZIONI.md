...
| Errore LLM **"Provider returned error"** | I modelli gratuiti di OpenRouter sono condivisi tra tutti gli utenti e a volte si sovraccaricano di un momento. L'app **riprova da sola una volta** dopo un attimo; se l'errore persiste: 1) aspetta 1-2 minuti e riprova, 2) o scegli un altro modello dall'elenco (ognuno è ospitato da un provider diverso). L'app mostra ora anche il **motivo esatto** del provider: se dice *quota/limit*, hai esaurito le 50 richieste giornaliere gratuite → torna il giorno dopo (o aggiungi 10$ una tantum per salire a 1000/giorno). In dubbio controlla lo storico richieste su https://openrouter.ai/activity . |
